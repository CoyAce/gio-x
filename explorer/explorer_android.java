...
public class explorer_android {
    ...
    private static Intent newOpenDocumentIntent(String mime) {
        ...
        if (mime == null || mime.isEmpty()) {
            ...
            // Convert file extensions to MIME types and determine the primary type
            String[] extensions = mime.split(",");
            intent.setType(getPrimaryMimeType(extensions));
    ...
    // Helper method to determine the primary MIME type from file extensions
    private static String getPrimaryMimeType(String[] extensions) {
        MimeTypeMap mimeMap = MimeTypeMap.getSingleton();
        boolean allImages = true;

        for (String ext : extensions) {
            String mimeType = mimeMap.getMimeTypeFromExtension(ext.trim().toLowerCase());
        ...
        // If all extensions are image types, use image/* for better UX
        if (allImages && extensions.length > 0) {
            return "image/*";
        }

        // Otherwise, return */*, let EXTRA_MIME_TYPES handle filtering
        return "*/*";
    }
